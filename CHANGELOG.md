...
- Optional `tls` block in `lucifer.json` serves the main gateway listener over HTTPS (#56). Three sources: `pem` (`certFile` / `keyFile` / optional `caFile`), `pfx` (a PKCS#12 bundle), and `windows-store` (Windows only — reads a certificate and its issuing chain out of `Cert:\<location>\<name>` through the native Windows CryptoAPI, selected by `dnsName` (the host name it was issued for, e.g. `pippo.codewrecks.com`), `thumbprint`, or `subject`; the bundle is held in memory, so no private key is written to disk). Optional `minVersion` is `TLSv1.2` (default) or `TLSv1.3`. Certificate paths resolve against the config file's directory, same rule as `dataDir` and alias `path` entries. Passphrases come from `LUCIFER_TLS_PASSPHRASE`, never from `lucifer.json`. Full contract: [`docs/specs/tls.md`](docs/specs/tls.md); design and rejected alternatives in ADR-014 and ADR-015.
  ...
  - **`store.dnsName` follows RFC 6125 wildcard matching**: `"dnsName": "pippo.codewrecks.com"` selects a `*.codewrecks.com` certificate (one left-most label only — never the bare parent domain or a deeper host), while the literal `*.codewrecks.com` still works. A certificate issued for the exact host outranks a covering wildcard, so installing one next to the other does not make the selector ambiguous.
  - **A selected certificate outside its validity window is logged**: it is still served, but startup warns with its thumbprint and dates rather than leaving every failing client handshake unexplained.
...
- `--log-format pretty|json` (env `LOG_FORMAT`) chooses the console log format, and `--log-file <path>` writes the JSON-lines log to a path of the operator's choosing, replacing `logFile` from `lucifer.json` (#60). The file is always JSON whatever the console shows. Documented under [Logging](docs/CONFIGURATION.md#logging).

### Changed
- **The console log is human-readable by default**, e.g. `[07:49:54] INFO: (app) Command gateway initialized` (#60). `pino-pretty` was a devDependency, so every `npm i -g` / `npx` install silently fell back to raw JSON lines; it now ships as a runtime dependency. The console format no longer depends on `NODE_ENV`, colour is used only on a TTY, and the Docker image sets `LOG_FORMAT=json` so container logs stay structured.
...
### Fixed
- **A listener that could not bind its port crashed with an uncaught stack trace** (#56). Both entrypoints now handle the server's `error` event and exit 1 with a message naming the port and how to move it (`--port`, `PORT`, `"port"`), with specific wording for `EACCES` and `EADDRINUSE`. Most likely hit with a `tls` block and no port set, where the listener defaults to the privileged `443`.
- CryptoAPI certificate handles are released when enumerating a store fails part-way, and the second `PFXExportCertStoreEx` call reports the same `0x8009000B` / `0x80090010` hints as the first (#56).
